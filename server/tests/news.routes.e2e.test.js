...
describe('News API', () => {
  ...
  it('should delete archived news item', async () => {
    ...
    expect(response.body).toBe(`New with id ${archivedNews.body._id} has been successfully deleted`)
  });
  ...
  it('should not delete not archived news item', async () => {
    ...
    expect(response.body).toBe(`It is not possible to delete new with id ${newNewsresponse.body._id}.`)
  });
});
